        ...
        q-item-label.q-py-md.row.justify-between.no-wrap.items-center
          .row.no-wrap.text-no-wrap
            input(
              ...
              @change="uploadMerchandiserNotebook($event)"
              hidden
            )
            //- createPlanogram
            ...
            OriginalButton.q-px-lg(
              :showLoad="false"
              @click="checkMerchandiserNotebook"
            ) Загрузить новый
</template>
<script>
...
export default {
  ...
  methods: {
    async checkMerchandiserNotebook () {
      if (this.project && this.project.value !== 'all') {
        this.$refs.merchandiserNotebook.click()
      } else this.$q.notify('Выберите проект')
    },
    uploadMerchandiserNotebook (e) {
      console.log(e)
      ...
      console.log(fd)
      api
        .call('createMerchandiserNotebook', {
