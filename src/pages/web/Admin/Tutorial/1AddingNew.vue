  ...
  div
      ...
      q-item
        q-item-section
          q-item-label.text-weight-550
            | Видеоматериал/фото/документы
          .q-pt-md.row.q-col-gutter-sm.justify-start
            template(v-if="tutorial.convertedVideos.length")
              .tutorial-card(
            ...
            template(v-if="tutorial.photos.length")
              .tutorial-card(
    ...
    q-dialog(
      v-model="modal.open"
        ...
        q-card(style="width:50%;max-height:100%;").flex.column.justify-between.no-wrap
          ...
          q-card-section.q-px-none.q-pt-md
            ...
            q-item
              q-item-section
                q-item-label
                  OriginalButton.w-full(
                    v-if="true"
                    color="red-2"
                    @click="postTutorials"
                  ) Разместить
                  InactiveButton.w-full(
                    v-else
                  ) Разместить
    q-dialog(
      position="bottom"
...
export default {
  ...
  mounted() {
    ...
    if (this.project) this.showTutorials();
  },
  ...
  methods: {
    ...
    async fileSelectEdit(type, files, file) {
      if (type === "photo") {
        console.log(files, file);
        const image = await this.fileDataURL(
          this.$refs.uploadPhotoEdit.files[0]
        );
    ...
    addVideoEdit() {
      ...
      const url = getVideoId(this.edit_videoUrl);
      if (url.id) {
        this.edit_tutorial.convertedVideos.push(
          `https://www.youtube.com/embed/${url.id}`
        );
      }
      // this.edit_tutorial.videos.push(this.edit_videoUrl)
